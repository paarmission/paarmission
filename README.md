...
| 페이지 | 파일 | 주요 내용 |
|--------|------|----------|
| 홈 (소개) | `index.html` | Hero + About + 페이지 바로가기 카드 + **중요 공지 팝업** |
| 신학적 기초 | `foundation.html` | 3대 기둥 (회복/자유/영광) + 5가지 핵심 원리 + 신앙고백서 14조 |
...
| **공지사항** | `notice.html` | **Notion DB 동적 게시판** + 전체/중요/공지사항 필터 탭 |

### 공지사항 시스템 (완료 — Worker 배포 + Notion DB ID 입력 필요)
- **Notion `Pa'ar Notice` DB** 연동 (`worker-code.js` `/notice` 엔드포인트)
- **공지 구분** 필드 (`공지사항` / `중요`) 기반 분기:
  - `중요` → 홈(`index.html`) **팝업 공지** (localStorage "오늘 하루 보지 않기")
  - 전체 → `notice.html` **게시판** (전체/중요/공지사항 필터 탭)
- `js/notion-notice.js` — 팝업 + 게시판 렌더링 + 상세 이미지 토글
- **⚠️ 필수**: Notion에서 `Pa'ar Notice` DB 생성 후 DB ID를 `worker-code.js` `DB_NOTICE` 변수에 입력 + Worker 재배포
...
- **Worker URL**: `https://dark-pine-8ced.superddj00.workers.dev`
  ...
  - `GET /notice` — **공지사항 DB 목록** (Pa'ar Notice)
...
partners.html           협력단체 (교회 / 선교사 / 기업)
notice.html             공지사항 게시판 (Notion 동적 연동)
sitemap.xml             검색엔진 사이트맵 (10개 URL)
worker-code.js          Cloudflare Worker 소스 [참고용 — 별도 배포 필요]
css/
  style.css             메인 스타일시트
js/
  main.js               공통 JS (navbar, 탭, ytModal, letterModal, imgZoom)
  ...
  notion-notice.js      Notion 공지사항 연동 (notice.html + index.html 팝업)
...
- [ ] **Cloudflare Worker 배포 (최우선)** — `worker-code.js`의 `/notice` 엔드포인트 포함 최신 코드를 `dark-pine-8ced` Worker에 배포 필요
- [ ] **Notion `Pa'ar Notice` DB ID 입력** — DB 생성 후 `worker-code.js`의 `DB_NOTICE = 'REPLACE_WITH_NOTION_DB_ID'` 교체 후 Worker 재배포
...
1. **Notion `Pa'ar Notice` DB 생성** (최우선):
   - Notion에서 DB 생성 → 속성: `이름`(제목), `공지 구분`(select: `공지사항`/`중요`), `내용`(텍스트)
   - DB ID 복사 → `worker-code.js` `DB_NOTICE` 변수에 붙여넣기
2. **Worker 배포** — `worker-code.js` 최신 코드를 `dark-pine-8ced.superddj00.workers.dev`에 재배포
3. **공지 작성 테스트** — Notion DB에 `중요` 공지 추가 → 홈 팝업 확인 / `notice.html` 게시판 확인
4. **Notion DB 채우기**: 협력 교회&단체 / 선교사 / 기업 DB에 데이터 입력 → `partners.html` 자동 반영
5. **계좌 정보**: `support.html`의 계좌 정보 업데이트
6. **배포**: Publish 탭에서 배포 → paarmission.org 반영
