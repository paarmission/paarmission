...
/* ── 첫 번째 이미지 URL 미리 가져오기 ───────────────────── */
async function fetchFirstImage(pageId) {
  try {
    var res = await fetch(WORKER_URL + '/blocks/' + pageId);
    if (!res.ok) return null;
    var data = await res.json();

    /* 커버가 있으면 커버를 우선 사용 */
    if (data.cover) {
      var src = data.cover.indexOf('http') === 0
        ? data.cover
        : WORKER_URL + data.cover;
      return src;
    }
    ...
    if (imgs.length > 0 && imgs[0].src) {
      var src = imgs[0].src.indexOf('http') === 0
        ? imgs[0].src
        : WORKER_URL + imgs[0].src;
      return src;
    }
  } catch(e) {
    console.error('[Notion] fetchFirstImage error:', pageId, e);
  }
  return null;
}

/* ── 편지 카드 HTML 생성 ──────────────────────────────────── */
function buildLetterCard(page, type, index, thumbSrc) {
  ...
  /* thumbSrc: fetchFirstImage로 미리 가져온 실제 이미지 우선,
     없으면 페이지 커버, 없으면 플레이스홀더 */
  var imgSrc = thumbSrc || getCoverUrl(page);
...
async function renderLetterGrid(gridEl, endpoint, type) {
  ...
  try {
    ...
    console.log('[Notion] ' + endpoint + ' pages:', pages.length, pages.map(function(p){ return p.id; }));
    ...
    gridEl.innerHTML = pages.map(function(p, i) {
      return '<div class="letter-card letter-card--skeleton" data-page-id="' + p.id + '">'
        + '<div class="letter-card-thumb"><div class="letter-skeleton-thumb"></div></div>'
        + '<div class="letter-card-body"><div class="letter-skeleton-line"></div><div class="letter-skeleton-line short"></div></div>'
        + '</div>';
    }).join('');

    /* 모든 페이지의 첫 번째 이미지를 병렬로 가져오기 */
    var thumbPromises = pages.map(function(p) { return fetchFirstImage(p.id); });
    var thumbs = await Promise.all(thumbPromises);

    /* 실제 카드로 교체 */
    gridEl.innerHTML = pages.map(function(p, i) {
      return buildLetterCard(p, type, i, thumbs[i]);
    }).join('');
