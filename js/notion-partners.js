...
(function () {
  ...
  function churchCard(page, idx) {
    ...
    const logo   = prop(p, '로고') || prop(p, 'Logo') || coverUrl(page) || '';
    const delay  = idx * 80;

    const logoHtml = logo
      ? `<img src="${esc(logo)}" alt="${esc(name)} 로고" loading="lazy" />`
    ...
    const linkHtml = link
      ? `<a href="${esc(link)}" target="_blank" rel="noopener" class="partner-link"><i class="fa-solid fa-arrow-up-right-from-square"></i> 바로가기</a>`
      : '';
  ...
  function missionaryCard(page, idx) {
    ...
    const desc    = prop(p, '소개') || prop(p, '설명') || prop(p, 'Description') || '';
    const photo   = prop(p, '사진') || prop(p, 'Photo') || coverUrl(page) || '';
    const delay   = idx * 80;

    const photoHtml = photo
      ? `<img src="${esc(photo)}" alt="${esc(name)} 사진" loading="lazy" />`
    ...
    return `
      <div class="partner-card partner-card--person" data-aos="fade-up" data-aos-delay="${delay}">
        <div class="partner-card-photo">${photoHtml}</div>
        <div class="partner-card-body">
          <h4 class="partner-name">${esc(name)}</h4>
          ${fieldHtml}
          ${orgHtml}
